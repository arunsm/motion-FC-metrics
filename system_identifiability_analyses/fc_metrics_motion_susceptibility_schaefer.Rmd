...
cbica_data="/cbica/home/tooleyu/projects/in_progress/arun_fc_metrics_motion/output/Schaefer_100_ICA_FIX/"
...
run1_newmets <- read.csv(paste0(cbica_data,"gsr_filter/new_metrics_modularity_schaefer__REST1_LR_020221.csv")) 
run2_newmets <- read.csv(paste0(cbica_data,"gsr_filter/new_metrics_modularity_schaefer__REST1_RL_020221.csv")) 
run3_newmets <- read.csv(paste0(cbica_data,"gsr_filter/new_metrics_modularity_schaefer__REST2_LR_020221.csv")) 
run4_newmets <- read.csv(paste0(cbica_data,"gsr_filter/new_metrics_modularity_schaefer__REST2_RL_020221.csv")) 
#add new Tikhonov metrics
run1_tikhonov <- read.csv(paste0(cbica_data,"gsr_filter/Tikhonov_new_modularity_schaefer__REST1_LR_061521.csv")) 
run2_tikhonov <- read.csv(paste0(cbica_data,"gsr_filter/Tikhonov_new_modularity_schaefer__REST1_RL_061521.csv")) 
run3_tikhonov <- read.csv(paste0(cbica_data,"gsr_filter/Tikhonov_new_modularity_schaefer__REST2_LR_061521.csv")) 
run4_tikhonov <- read.csv(paste0(cbica_data,"gsr_filter/Tikhonov_new_modularity_schaefer__REST2_RL_061521.csv")) 
```
...
alldata1 <- merge(alldata1, run1_pcor, by = "subject", all = TRUE) %>% dplyr::select(-c(avgweight_Spearman, modul_Spearman))
...
alldata2 <- merge(alldata2, run2_pcor, by = "subject", all = TRUE)%>% dplyr::select(-c(avgweight_Spearman, modul_Spearman))
...
alldata3 <- merge(alldata3, run3_pcor, by = "subject", all = TRUE)%>% dplyr::select(-c(avgweight_Spearman, modul_Spearman))
...
alldata4 <- merge(alldata4, run4_pcor, by = "subject", all = TRUE)%>% dplyr::select(-c(avgweight_Spearman, modul_Spearman))
...
## Modularity and motion, WITHOUT controlling for age and sex, just avg weight
```{r not controlling for covariates, just avgweight modul, echo=TRUE}
metrics=c("Pearson", "Spearman", "PartialCorrelation", "TikhonovPartialCorrelation", "Coherence", "WaveletCoherence", "MutualInformation","MutualInformationTime")
#calculate the relationship between modularity and motion controlling for avg weight
for (i in 1:4){
  assign("data",get(paste0("alldata",i)))
...
```{r bpfilt motion, controlling for avgweight and covariates modul, echo=TRUE}
...
p<-ggplot(all, aes(x=metrics, y=value, color=metrics)) + 
  geom_jitter(position=position_jitter(0), cex=2.5)
p + theme_classic() + stat_summary(fun.data="mean_sdl",fun.args = list(mult = 1) ,
                 geom="pointrange", shape=95, cex=1) + labs(title="100-node Yeo parcellation",x="", y = "Partial correlation of Q with bandpass-filtered motion")+ theme(plot.title = element_text(hjust = 0.5), legend.position = "none")+ theme(axis.text=element_text(size=14),
        axis.title=element_text(size=14)) +scale_color_manual(values=c("#0072BD", '#006432','#EB008B',"#D95319", "#EDB120", "#7E2F8E", "#77AC30", "#4DBEEE"))+geom_hline(yintercept = 0)
```
...
run1_newmets=readMat(paste0(dropbox_data, "apriori/new_metrics_modul_yeo_partition1.mat"))
run1_newmets <- data.frame(matrix(unlist(run1_newmets$modul, use.names = FALSE), nrow=length(run1_newmets$modul[[1]]), byrow=F))
colnames(run1_newmets) =c("modul_Spearman", "modul_TikhonovPartialCorrelation")
run2_newmets=readMat(paste0(dropbox_data, "apriori/new_metrics_modul_yeo_partition2.mat"))
run2_newmets <- data.frame(matrix(unlist(run2_newmets$modul, use.names = FALSE), nrow=length(run2_newmets$modul[[1]]), byrow=F))
colnames(run2_newmets) =c("modul_Spearman", "modul_TikhonovPartialCorrelation")
run3_newmets=readMat(paste0(dropbox_data, "apriori/new_metrics_modul_yeo_partition3.mat"))
run3_newmets <- data.frame(matrix(unlist(run3_newmets$modul, use.names = FALSE), nrow=length(run3_newmets$modul[[1]]), byrow=F))
colnames(run3_newmets) =c("modul_Spearman", "modul_TikhonovPartialCorrelation")
run4_newmets=readMat(paste0(dropbox_data, "apriori/new_metrics_modul_yeo_partition4.mat"))
run4_newmets <- data.frame(matrix(unlist(run4_newmets$modul, use.names = FALSE), nrow=length(run4_newmets$modul[[1]]), byrow=F))
colnames(run4_newmets) =c("modul_Spearman", "modul_TikhonovPartialCorrelation")

mydir="~/Documents/projects/in_progress/arun_fc_metrics_motion/data/subjData/Motion_S1200/rfMRI_REST1_LR/"
motiondata1 <- read.csv(paste0(mydir,"/rfMRI_REST1_LR.csv"), header = FALSE, col.names = c("subject", "motion"))
modul_data1 <- data.frame(run1_pcor, run1_data)
rerank_alldata1 <- merge(modul_data1, motiondata1, by="subject", all=TRUE)%>% dplyr::select(c(-modul_Spearman))
...
rerank_alldata2 <- merge(modul_data2, motiondata2, by="subject", all=TRUE)%>% dplyr::select(c(-modul_Spearman))
...
rerank_alldata3 <- merge(modul_data3, motiondata3, by="subject", all=TRUE)%>% dplyr::select(c(-modul_Spearman))
...
rerank_alldata4 <- merge(modul_data4, motiondata4, by="subject", all=TRUE)%>% dplyr::select(c(-modul_Spearman))
...
cbica_data= "/cbica/home/tooleyu/projects/in_progress/arun_fc_metrics_motion/output/Schaefer_100_ICA_FIX/absvalue_PartialCorr/"
new_data="/cbica/home/tooleyu/projects/in_progress/arun_fc_metrics_motion/output/Schaefer_100_ICA_FIX/abs_value_PearsonSpearman/"
...
run1_newmets=read.csv(paste0(new_data, "new_metrics_absvalue_modularity_schaefer__REST1_LR_020321.csv")) %>% rename(.,TikhonovPartialCorrelation=modul_TikhonovPartialCorrelation, Spearman=modul_Spearman)
...
run2_newmets=read.csv(paste0(new_data, "new_metrics_absvalue_modularity_schaefer__REST1_RL_020321.csv")) %>% rename(.,TikhonovPartialCorrelation=modul_TikhonovPartialCorrelation, Spearman=modul_Spearman)
...
run3_newmets=read.csv(paste0(new_data, "new_metrics_absvalue_modularity_schaefer__REST2_LR_020321.csv")) %>% rename(.,TikhonovPartialCorrelation=modul_TikhonovPartialCorrelation, Spearman=modul_Spearman)
...
run4_newmets=read.csv(paste0(new_data, "new_metrics_absvalue_modularity_schaefer__REST2_RL_020321.csv")) %>% rename(.,TikhonovPartialCorrelation=modul_TikhonovPartialCorrelation, Spearman=modul_Spearman)
...
rerank_alldata2 <- merge(rerank_alldata2, motiondata2, by = "subject", all = TRUE) %>% dplyr::select(c(-Spearman))
rerank_alldata2 <- cbind(rerank_alldata2,run2_newmets)
