---
title: "FC Metrics Susceptibility to Motion- Schaefer ICA FIX with Alternative Pipelines"
output:
  html_document:
    theme: united
    toc: yes
...
run1_data=read.csv(paste0(data_dir, "/modularity_yeo_100__REST1_LR_071120.csv"))
run2_data=read.csv(paste0(data_dir, "/modularity_yeo_100__REST1_RL_071120.csv"))
run3_data=read.csv(paste0(data_dir, "/modularity_yeo_100__REST2_LR_071120.csv"))
run4_data=read.csv(paste0(data_dir, "/modularity_yeo_100__REST2_RL_071120.csv"))
...
run1_newmets <- read.csv(paste0(cbica_data,pipeline,"/new_metrics_modularity_schaefer__REST1_LR_020221.csv")) 
run2_newmets <- read.csv(paste0(cbica_data,pipeline,"/new_metrics_modularity_schaefer__REST1_RL_020221.csv")) 
run3_newmets <- read.csv(paste0(cbica_data,pipeline,"/new_metrics_modularity_schaefer__REST2_LR_020221.csv")) 
run4_newmets <- read.csv(paste0(cbica_data,pipeline,"/new_metrics_modularity_schaefer__REST2_RL_020221.csv")) 
#add new Tikhonov metrics
run1_tikhonov <- read.csv(paste0(cbica_data,pipeline,"/Tikhonov_new_modularity_schaefer__REST1_LR_061521.csv")) 
run2_tikhonov <- read.csv(paste0(cbica_data,pipeline,"/Tikhonov_new_modularity_schaefer__REST1_RL_061521.csv")) 
run3_tikhonov <- read.csv(paste0(cbica_data,pipeline,"/Tikhonov_new_modularity_schaefer__REST2_LR_061521.csv")) 
run4_tikhonov <- read.csv(paste0(cbica_data,pipeline,"/Tikhonov_new_modularity_schaefer__REST2_RL_061521.csv")) 
```
...
p + theme_classic() + stat_summary(fun.data="mean_sdl",fun.args = list(mult = 1) ,
                 geom="pointrange", shape=95, cex=1) + labs(title=paste0("100-node Yeo parcellation ", pipeline),x="", y = "Modularity quality index (Q)")+ theme(plot.title = element_text(hjust = 0.5), legend.position = "none")+ theme(axis.text=element_text(size=14),
...
p + theme_classic() + stat_summary(fun.data="mean_sdl",fun.args = list(mult = 1) ,
                 geom="pointrange", shape=95, cex=1) + labs(title=paste0("100-node Yeo parcellation ", pipeline),x="", y = "Partial correlation of Q with motion")+ theme(plot.title = element_text(hjust = 0.5), legend.position = "none")+ theme(axis.text=element_text(size=14),
