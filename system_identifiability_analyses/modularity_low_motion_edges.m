...
fc_metrics={'Spearman', 'TikhonovPartialCorrelation'}; %add two new metrics
...
save(fullfile(outdir, strcat('new_metrics_modul_nomotionedges_run',int2str(i))), 'modul')
save(fullfile(outdir, strcat('new_metrics_numcommunities_nomotionedges_run',int2str(i))), 'num_communities')
save(fullfile(outdir, strcat('new_metrics_avgnumcommunities_nomotionedges_run',int2str(i))), 'avgnumcommunities')
save(fullfile(outdir, strcat('new_metrics_avgweight_nomotionedges_run',int2str(i))), 'avgweight')
try
    ...
    filename=strcat('new_metrics_modularity_nomotionedges_yeo_',run, '020321.csv') %need to figure out how to get the headers to work.
