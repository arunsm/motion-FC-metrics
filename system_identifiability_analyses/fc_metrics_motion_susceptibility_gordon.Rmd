...
alldata1 <- cbind(alldata1, run1_newmets)
...
alldata2 <- cbind(alldata2, run2_newmets)
...
alldata3 <- cbind(alldata3, run3_newmets)
...
alldata4 <- cbind(alldata4, run4_newmets)
...
for (i in 1:4){
  ...
  for (l in 1:6){
    assign("metric", metrics[l])
    temp <- data %>% dplyr::select(.,matches(paste0("_",metric,"$")),matches("bp_motion"), matches("Age_numeric"),matches("Gender_numeric"))
    print(colnames(temp))
    temp2 <- cbind(temp[1], temp$Age_numeric, temp$Gender_numeric)
...
rerank_alldata1 <- cbind(rerank_alldata1,run1_newmets)
...
rerank_alldata2 <- cbind(modul_data2,run2_newmets)
...
rerank_alldata3 <- cbind(modul_data3,run3_newmets)
...
rerank_alldata4 <- cbind(modul_data4,run4_newmets)
...
rerank_alldata2 <- cbind(rerank_alldata2,run2_newmets)
...
rerank_alldata3 <- cbind(rerank_alldata3,run3_newmets)
...
rerank_alldata4 <- cbind(rerank_alldata4,run4_newmets)
