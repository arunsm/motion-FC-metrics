...
rerank_alldata1 <- rerank_alldata1 %>% filter(., num_communitiesPartialCorrelation != 0)
#recode age to be the median of the bins that are provided
...
rerank_alldata2 <- rerank_alldata2 %>% filter(., num_communitiesPartialCorrelation != 0)
#recode age to be the median of the bins that are provided
...
rerank_alldata3 <- rerank_alldata3 %>% filter(., num_communitiesPartialCorrelation != 0)
...
rerank_alldata4 <- rerank_alldata4 %>% filter(.,num_communitiesPartialCorrelation != 0)
...
for (i in 1:4){
  ...
  averagemodulorig <- summary(dataorig$modul_Pearson)[4]
  assign("data",get(paste0("rerank_alldata",i)))
  averagemodulrerank <- summary(data$Pearson.WaveletCoherence)[4] #look just at which ones are higher, the original or the re-ranked?
